...
class quantritin extends goc{
   ...
   WHERE idTL=$idTL";
    ...
    function DemTheLoaiTrongTin($idTL){
        settype($idTL,"int");
        $sql = "SELECT COUNT(*) as count FROM tin WHERE idTL=$idTL";
        $kq= $this->db->query($sql) ;
        if(!$kq) die( $this->db->error);
        $temp = $kq->fetch_assoc();
        return $temp['count'];
    ...
    function ListLoaiTinMoi(){
        $sql="SELECT idLT,TenLT,ThuTu,AnHien,lang FROM loaitin ORDER BY ThuTu";
        $kq = $this->db->query($sql) ;
        if(!$kq) die( $this-> db->error);
        return $kq;
    }
   ...
   WHERE idLT=$idLT";
    ...
    function DemLoaiTinTrongTin($idLT){
        settype($idTL,"int");
        $sql = "SELECT COUNT(*) as count FROM tin WHERE idTL=$idLT";
        $kq= $this->db->query($sql) ;
        if(!$kq) die( $this->db->error);
        $temp = $kq->fetch_assoc();
        return $temp['count'];
  ...
  tags='$tags', lang='$lang', idUser= $idUser";
    ...
    function Tin_Xoa($idTin){
        settype($idTin,"int"); $sql="DELETE FROM tin WHERE idTin=$idTin";
        $kq= $this->db->query($sql) ;
        if(!$kq) die( $this-> db->error);
    }
    function ListUser(){
        $sql="SELECT idUser,HoTen,Username,Password,DiaChi,Dienthoai,Email FROM users ORDER BY idUser";
        $kq = $this->db->query($sql) ;
        if(!$kq) die( $this-> db->error);
        return $kq;
    }
